   ...
   public App()
    {
        ...
        _settingsManager = new SettingsManager<UserSettings>(Path.Combine(SettingsDir, "userSettings.json"));
        UserSettings = _settingsManager.LoadSettings();
        ...
        if (ApplicationDeployment.IsNetworkDeployed) {
            Log.Info(JsonSerializer.Serialize(ApplicationDeployment.CurrentDeployment));
        } else {
            Log.Info("Not network deployed");
        }
    }
    ...
    private void Application_Startup(object sender, StartupEventArgs e)
    {
        ...
        Log.Info($"App {VersionString} started");
        ...
        Manager.Start(new Monitor(), new LiveMonitor(), new Analyzer(), new HeroesProfile.Uploader.Common.Uploader(), new LiveProcessor(Manager.PreMatchPage));
    }
    ...
    {
        Log.Error(e, $"Unhandled {type} exception");
        try {
